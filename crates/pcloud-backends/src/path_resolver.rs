...
fn split_parent(path: &str) -> (String, String) {
    // SAFETY: all paths entering this function have been normalised by
    // `normalise_path`, which guarantees a leading '/' in every returned
    // value; therefore `rfind('/')` always returns `Some(_)`.
    let idx = path
        .rfind('/')
