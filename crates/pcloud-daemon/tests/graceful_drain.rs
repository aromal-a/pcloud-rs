...
fn bootstrap_test_shell() -> pcloud_daemon::RuntimeShell {
    // Use `/tmp` (not `std::env::temp_dir()`) on Unix so the Unix-socket
    // path stays under SUN_LEN on macOS, where the per-user tempdir
    // `/var/folders/.../T/` alone eats 49 chars. On Windows SUN_LEN
    // doesn't apply (named pipes aren't path-length-bounded) so the
    // system tempdir is fine and `/tmp` wouldn't satisfy
    // `path.is_absolute()`.
    use std::sync::atomic::{AtomicU64, Ordering};
    static SEQ: AtomicU64 = AtomicU64::new(0);
    let nonce = SEQ.fetch_add(1, Ordering::Relaxed);
    #[cfg(unix)]
    let root_base = std::path::PathBuf::from("/tmp");
    #[cfg(not(unix))]
    let root_base = std::env::temp_dir();
...
// Cross-platform: the drain test flips a shutdown flag and requires
// the serve loop to break out of a pending `accept` promptly. On Unix
// `set_accept_timeout` installs a periodic `SO_RCVTIMEO` wake-up; on
// Windows the daemon's serve loop now spawns a scoped
// shutdown-watcher thread that signals a manual-reset cancel event
// via `BoundIpcServer::request_shutdown`, and the overlapped
// `ConnectNamedPipe` path unblocks via `CancelIoEx` and returns
...
// Cross-platform: see rationale on `drain_admits_status_probes_and_rejects_new_traffic`.
#[test]
